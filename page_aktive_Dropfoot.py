...
from quest_SysUsability_Dropfoot import QuestUsability

__author__ = 'Andreas P. Cuny & Carole Pauli'

class PageAktive(object):
    """
    ...
    def __init__(self, *args):
        ...
        if len(args) == 5:
            ...
            self.questUsabilityAktive.usability = self.tr.questUsabilityAktive.usability
            
        part_ID = str(self.meta.study_metadata.children[1].value)
        
        display(Markdown("# " + self.textform.messkonditionen[1]))
        display(Markdown(self.textform.meas_text[0]))
        display(Markdown(self.textform.meas_text[1]))
        if len(args) == 5:
            self.aktivStat = self.tr.aktivStat
            self.aktivStat.show()
        else:
            self.aktivStat = TrialNoTime(part_ID + '_asc_static', 3, self.screen)
            display(self.aktivStat.output)
        ...
        if len(args) == 5:
            self.asc_xw = self.tr.asc_xw
            self.asc_xw.show()
        else:
            self.asc_xw = TrialWithTime(part_ID + '_asc_xw', 10, True, self.textform, self.screen)
            display(self.asc_xw.output)
        display(Markdown(self.textform.meas_text[9]))
        if len(args) == 5:
            self.asc_fw = self.tr.asc_fw
            self.asc_fw.show()
        else:
            self.asc_fw = TrialWithTime(part_ID + '_asc_fw', 5, True, self.textform, self.screen)
            display(self.asc_fw.output)
        ...
        self.druck = ["# Bewertung des wahrgenommenen Drucks", 
                      self.textform.messkonditionen[1], 
        ...
        if len(args) == 5:
            display(self.questUsabilityAktive.usability)
        else:
            self.init_SystemUsability()
            
        display(Markdown("---"))
        display(Markdown("# Kommentare / Anmerkungen"))
        display(Markdown("Fragen, wie Patienten das Gewicht der Schiene beurteilen würden, sowohl am Rücken wie auch am Fuss. Weitere Anmerkungen?"))
        self.komm_aktive = widgets.Textarea(value='', description='', disabled=False, style={'description_width':'initial'}, layout=widgets.Layout(width='auto'))
        display(self.komm_aktive)
    ...
    def init_SystemUsability(self,*args):
        self.questUsabilityAktive = QuestUsability(self.textform.messkonditionen[1],self.screen)